        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                ...
                <fieldset class="fieldset">
                    ...
                    <select name="family_id" class="select input w-full border border-gray-300">
                        ...
                        @foreach ($families as $family) 
                            <option value="{{ $family->id }}" @selected($family->id==old('family_id', $category->family_id ?? ''))>{{$family->name}}</option>
                        @endforeach
                    </select>
                </fieldset>
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="text" placeholder="Nombre de la familia" name="name" value="{{ old('name', $category->name) }}" class="input w-full border border-gray-300" />
                </fieldset>
